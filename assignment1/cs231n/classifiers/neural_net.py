...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    softmax_scores = np.exp(stabilized_scores)/np.sum(np.exp(stabilized_scores),axis=1,keepdims=True)
    ...
    # Backprop for dL/dY with cross entropy loss
    ...
    #backprop for W2 
    dW2 = np.dot(h.T,dSoftmax)
    dW2 += reg*2*W2
    grads['W2'] = dW2
    
    #backprop for ReLU with x2
    dX2 = np.dot(dSoftmax,W2.T)
    ReLU_mask = np.where(h<0.00000000000000001,h,1)
    dReLU = dX2*ReLU_mask
